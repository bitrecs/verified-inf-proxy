...
async def test_ping_provider():
    ping_result = LLMProviderStats.ping_provider_html(LLMProvider.CLAUDE)
    assert ping_result is not None
    print(f"CLAUDE ping result: {ping_result}")
...
async def test_call_claude():    
    ...
    response = await call_proxy_server_with_signing(BASE_URL, request, miner_keypair.ss58_address, miner_keypair, "CLAUDE", CLAUDE_API_KEY)
