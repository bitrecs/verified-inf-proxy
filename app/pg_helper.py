...
class PGHandler:
    ...
    def select_signed_responses(self, limit=100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with conn.cursor() as cur:
                sql = f"""
                SELECT hotkey, timestamp, model, duration, signature, provider, 
                       created_at, completion_response, prompt_tokens, completion_tokens, total_tokens 
                FROM {TABLE_NAME} 
    ...
    def select_signed_responses_formix_since(self, since_date: datetime, limit=100) -> List[Dict[str, Any]]:
        """Select signed responses slim version"""
        results = []
        try:
            conn = self.connect()
            with conn.cursor() as cur:
                sql = f"SELECT hotkey, model FROM {TABLE_NAME} WHERE created_at >= %s ORDER BY created_at DESC LIMIT %s"
                cur.execute(sql, (since_date, limit))
                rows = cur.fetchall()
                columns = [desc[0] for desc in cur.description]
                for row in rows:
